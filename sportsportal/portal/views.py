...
class RegisterManagerView(APIView):
    def post(self, request):
        ...
        try:
            if game == "Badminton":
                BadmintonManager.objects.update_or_create(
                    user=user,
                    defaults={'team_name': team_name}
                )
            elif game == "Cricket":
                CricketManager.objects.update_or_create(
                    user=user,
                    defaults={'team_name': team_name}
                )
            elif game == "Football":
                FootballManager.objects.update_or_create(
                    user=user,
                    defaults={'team_name': team_name}
                )
            else:
            ...
            return Response({'error': f"Manager for user '{username}' already exists."}, status=HTTP_400_BAD_REQUEST)

        return Response({'message': f"Manager registered successfully for {game}."}, status=HTTP_201_CREATED)
